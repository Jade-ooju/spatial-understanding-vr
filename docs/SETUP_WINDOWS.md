...
conda activate sam3
cd D:\OOJU\Projects\SpatialUnderstanding\spatial-understanding-vr
python -c "import sys; sys.path.insert(0, 'scripts'); from physics_estimator import PhysicsEstimator; import numpy as np; img = np.zeros((224, 224, 3), dtype=np.uint8); y, x = np.ogrid[:224, :224]; mask = (x-112)**2 + (y-112)**2 <= 50**2; img[mask] = 255; est = PhysicsEstimator(); print('VLM initialized:', est.initialized)"
```
