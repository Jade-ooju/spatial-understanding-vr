...
**Note:** This document is for setting up the GitHub repository, not for setting up the pipeline itself. For pipeline setup instructions, see:
- **Windows**: [SETUP_WINDOWS.md](SETUP_WINDOWS.md)
- **Linux**: Use `scripts/install_sam3.sh`
- **Quick Start**: [QUICK_REFERENCE.md](QUICK_REFERENCE.md)

## What's Included

This repository contains:
- All custom video pipeline scripts (in `scripts/`)
...
After cloning this repository, users will need to:

1. Set up the environment (see [SETUP_WINDOWS.md](SETUP_WINDOWS.md) for Windows or use `scripts/install_sam3.sh` for Linux)
...
4. Run the pipeline: `python scripts/video_pipeline.py --video <path> --prompt "spoon"`
